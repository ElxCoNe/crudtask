...
const totalTask = document.querySelector("#mTotal");
...
async function openEditModal(id) {
    try {
        ...
        console.error(err);
        Swal.fire("Error", "No se pueden cargar las tareas", "error");
    }
}
...
async function loadTasks() {
    ...
    try {
        const listTasks = await apiGet(`/tasks?userId=${sessionData.id}`);
        renderTasks(listTasks);
    } catch (error) {
        console.log('Error contacta al administrador:', error);
    }
}
